    ...
    public class CommandManager : MonoBehaviour
    {
        ...
        private CoroutineWrapper ExecuteSubCommand(string commandName, string[] args)
        {
            ...
            if (subDatabases.ContainsKey(databaseName))
            {
                ...
                if (command != null)
                {
                    return StartProcess(commandName,
                        command, args);
                }
                else
                {
                    Debug.LogError($"No command called '{subComandName}' was found in subdatabase '{databaseName}'");
                    return null;
                }
            }
            
            string characterName = databaseName;
            //если мы сделали это здесь, тогда стоит попробовать запустить как команду персонажа
            ...
            Debug.LogError($"No sub databasae called '{databaseName}' exists! Command '{subComandName}'could not be run.");
            return null;
        }
